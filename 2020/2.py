...
def solve(cases=TESTS):
    ...
    for i in range(len(cases)):
        case = cases[i]
        results = [i.start() for i in re.finditer(case[0][2], case[1])]
        # print(results, len(results), case[0])
        if len(results) >= case[0][0] and len(results) <= case[0][1]:
            valid += 1
            continue
        invalid += 1
    return (valid, invalid)

def solve2(cases=TESTS):
    valid = 0
    invalid = 0
    for i in range(len(cases)):
        case = cases[i]
        print(case)
        case_length= len(case[1])
        if case[0][0] < 0 or case[0][0] > case_length:
            invalid += 1
            print(case[0][0], case_length)
            print("start is less than length")
            continue
        
        first = case[1][case[0][0] - 1]
        print(first)
        if case[0][1] > case_length:
            if first == case[0, 2]:
                invalid += 1
            print("end is less than length")
            continue

        second = case[1][case[0][1] - 1]
        print(second)
        if first == second:
            invalid += 1
            print("Positions match")
            continue
        if first != case[0][2] and second != case[0][2]:
            invalid += 1
